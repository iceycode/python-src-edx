...
#for molecules clearning out
# Exponential Decay of molecule
# shows half life


#NO randomness in this, so not monte carlo sim
...
# pylab.title('Exponential Decay')
# pylab.semilogy() #straight line, one way to check if exponential distribution
# pylab.show()
    ...
    for t in range(steps):
        ...
        for m in range(numRemaining[-1]):
            if random.random() <= clearProb: 
                numLeft -= 1
        #at every 100 steps, molecules clones themselves (double)
        if t != 0 and t %100 == 0:
            numLeft+=numLeft
